...
import {  Avatar, Button, Card, Title, Paragraph, Searchbar, DefaultTheme, List, RadioButton } from 'react-native-paper';
...
const MenuItemScreen = ({ navigation , route }) => {
        ...
        const userRef = firebase.database().ref(`/users/uE1OWGdOQRfPna29DnQ9yGUTvIF3`);
            userRef.once('value', (snapshot) => {
              if (snapshot.val() !== null) {
                // console.log(snapshot.val().expoToken);
                setData({ 
                  ...data, 
  ...
  const onSearchStateChange = (results) => {
    ...
    // const sendNotification = (title, body, type, image) => {
    //     const { currentUser } = firebase.auth();
    //     if (currentUser !== null) {
    //       sendPushNotification ( 
    //             data.managmentExpoToken,
    //             title,
    //             body,
    //             type,
    //             image,
            ...
            // if (data.val().uidUser !== undefined && data.val().uidUser.toString() !== currentUser.uid.toString()) {
              ...
              const userRef = firebase.database().ref(`/users/${user.uid}`);
                userRef.once("value", (snap) => {
                    ...
                    sendPushNotification('New Item Pic', 'A user submitted a new item for your approval.', 'NewItemPic', data.requestFoodPic, user.uid, snapShot.key);
                  });
                });
                } 
            })
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
              <Modal  
                ...
                onBackdropPress={() => { cancelUpdateNewItem(); }} 
                backdropColor='black'
                useNativeDriver={true}
                backdropOpacity={0.3}
                hideModalContentWhileAnimating
              ...
              >
              <ScrollView style={{ backgroundColor: 'white', borderRadius: 35 }}>
                <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                  <View style={{ marginTop: 10 }}>
                    <Image
                      style={{ width: 100 , height: 100, marginTop: 35 }}
                  ...
                  <View style={{ marginTop: 10, backgroundColor: '#ffffff', paddingBottom: 8, justifyContent: 'flex-start' }}>
                  <View style={{ marginTop: 10, backgroundColor: '#ffffff', paddingBottom: 8, justifyContent: 'flex-start', flexDirection: 'row', marginLeft: 15 }}>
                    <Title style={{ color: 'black', fontFamily: 'MontserratSemiBold', fontSize: 14, marginTop: 10 }}>Item Name: </Title>
                    <Title style={{ color: 'black', fontFamily: 'MontserratBold', fontSize: 15, marginTop: 10 }}>{data.requestFoodName}</Title>
                  </View>
                  <View style={{ marginBottom: 10, marginTop: -25 }}>
                    ...
                    >
                      <List.Accordion
                        ...
                        onPress={() => { setData({ ...data, toggleFoodType: !data.toggleFoodType }) }}
                      >
                        <TouchableOpacity
                          style={{ width: width * 0.60 , marginLeft: 30 }}
                          onPress={() => { setData({ ...data, foodRequestedType: 'Entrée' }) }}
                        >
                          <List.Item 
                            style={{ borderTopLeftRadius: 15, borderTopRightRadius: 15, backgroundColor: 'rgba(0, 0, 0, 0.02)', marginLeft: -25, elevation: 1 }} 
                            title=" Entrée"
                            titleStyle={{
                              fontSize: 14,
                              marginTop: -5,
                            ...
                            left={props => 
                              <View style={{ flexDirection: 'row' }}> 
                                <RadioButton
                                  value="Entrée"
                                  status={data.foodRequestedType === 'Entrée' ? 'checked' : 'unchecked'}
                                  color={"#C90611"}
                        ...
                        <TouchableOpacity
                          style={{ width: width * 0.60 , marginLeft: 30 }}
                          onPress={() => { setData({ ...data, foodRequestedType: 'Appetizer' }) }}
                        >
                          <List.Item 
                            style={{ backgroundColor: 'rgba(0, 0, 0, 0.02)', marginLeft: -25, elevation: 1 }} 
                            title=" Appetizer"
                            titleStyle={{
                              fontSize: 14,
                              marginTop: -5,
                            ...
                            left={props => 
                              <View style={{ flexDirection: 'row' }}> 
                                <RadioButton
                                  value="Appetizer"
                                  status={data.foodRequestedType === 'Appetizer' ? 'checked' : 'unchecked'}
                                  color={"#C90611"}
                                  uncheckedColor={"#C90611"}
                                  // onPress={() => {}}
                                />
                        ...
                        <TouchableOpacity
                          style={{ width: width * 0.60 , marginLeft: 30 }}
                          onPress={() => { setData({ ...data, foodRequestedType: 'Dessert' }) }}
                        >
                          <List.Item 
                            style={{ backgroundColor: 'rgba(0, 0, 0, 0.02)', marginLeft: -25, elevation: 1 }} 
                            title=" Dessert"
                            ...
                            left={props => 
                              <View style={{ flexDirection: 'row' }}> 
                                <RadioButton
                                  value="Dessert"
                                  status={data.foodRequestedType === 'Dessert' ? 'checked' : 'unchecked'}
                                  color={"#C90611"}
                                  uncheckedColor={"#C90611"}
                                  // onPress={() => {}}
                                />
                        ...
                        <TouchableOpacity
                          style={{ width: width * 0.60 , marginLeft: 30 }}
                          onPress={() => { setData({ ...data, foodRequestedType: 'Drink' }) }}
                        >
                          <List.Item 
                            style={{ backgroundColor: 'rgba(0, 0, 0, 0.02)', borderBottomLeftRadius: 15, borderBottomRightRadius: 15, marginLeft: -25, elevation: 1 }} 
                            title=" Drink"
                            titleStyle={{
                              fontSize: 14,
                              marginTop: -5,
                            ...
                            left={props => 
                              <View style={{ flexDirection: 'row' }}> 
                                <RadioButton
                                  value="Drink"
                                  status={data.foodRequestedType === 'Drink' ? 'checked' : 'unchecked'}
                                  color={"#C90611"}
                                  uncheckedColor={"#C90611"}
                                  // onPress={() => {}}
                                />
                ...
                </View>
                {
                  data.requestFoodPic === '' ?
                    <View style={{ height: width * 0.41, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-evenly', marginTop: 10 }}>
                      <View style={{flex: 1,  marginTop: 2, alignItems: 'center' }}>
                        <TouchableOpacity 
                        onPress={() => {useCameraHandlerNewItem()}}
                        > 
                          <Image
                      ...
                      <View style={{flex: 1,  marginTop: 2, alignItems: 'center' }}>
                        <TouchableOpacity 
                        onPress={() => {useLibraryHandlerNewItem()}}
                        >
                          <Image
                ...
                <TouchableOpacity onPress={() => { postNewItem() }} disabled={data.requestFoodPic === '' ? true : false}>
                  <View style={{ marginTop: data.requestFoodPic === '' ? -20 : 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }}>
                    <LinearGradient colors={ data.requestFoodPic === '' ? ['#cccccc', '#cccccc', '#cccccc'] : ['#fb8389', '#f70814', '#C90611']} style={styles.linearGradient}>
                      <Text style={styles.buttonText}>Post Photo</Text>
                    </LinearGradient>
                  </View>
                </TouchableOpacity>
                <View style={{ marginTop: 5, justifyContent: 'center', alignItems: 'center', marginBottom: 15 }}>
                  <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center' }} onPress={() => { cancelUpdateNewItem() }} >
                    <Title style={{ color: 'black', fontSize: 16, fontFamily: 'Montserrat' }}>Cancel</Title>
                  </TouchableOpacity>
                </View>
              </ScrollView>
                ...
                <View style={{ justifyContent: 'center', alignItems: 'center', marginBottom: (hits.length - 1) === index ? 100 : 10 , marginTop: 20 }}>
                  <Card elevation={10} style={{ width: width * 0.90, borderRadius: 15 }}>
                    <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 0 }}>
                      {/* <Image
                        style={{ resizeMode: "contain", width: width * 0.90, height: height * 0.4, borderTopLeftRadius: 20, borderTopRightRadius:20 }} 
                        source={{ uri: item.foodInfo.image === oldImage ? newImage : item.foodInfo.image }}
                      /> */}
                      <Card.Cover style={{ resizeMode: "contain", width: width * 0.90, height: height * 0.25,borderTopLeftRadius: 15, borderTopRightRadius:15 }} source={{ uri: item.foodInfo.image === oldImage ? newImage : item.foodInfo.image }} />
                      {
                        ...
                        >
                            <Button style={{ flex: 1, backgroundColor: '#C90611', borderRadius: 10, elevation: 3, marginBottom: 40 }} >
                    ...
                    <View style={{ flexDirection: 'row', width: width * 0.90, justifyContent: 'flex-start', marginTop: item.foodInfo.image === oldImage || item.foodInfo.image === newImage ? -30 : 0 }}>
                      ...
                      <View style={{ alignItems: 'flex-start', flexDirection: 'row', marginLeft: -55, justifyContent: 'flex-start' }}>
                        {
                          ...
                          <View style={{ alignItems: 'center', marginLeft: 15, flexDirection: 'row', marginTop: 0 }}>
                            <View>
...
const SearchBox = connectSearchBox(({ onFocus, onBlur, refine, currentRefinement, navigation }) => {
      ...
      <Searchbar
        ...
        onIconPress={() => { 
          ...
          if (searchState.resultsLength === 0) {
            navigation.navigate('MenuItem', { noResultsReq: true, requestFoodName: currentRefinement });
          }
        }}
        onSubmitEditing={() => { 
          // searchState = Object.assign({ ...searchState, noResultsReq: true }); 
        ...
        onChangeText={
          (text) => { 
            ...
            refine(text); 
            // searchState = Object.assign({ ...searchState, searchingWord: text, noResultsReq: true }); 
