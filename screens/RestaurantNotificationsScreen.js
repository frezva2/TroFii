...
const RestaurantNotificationsScreen = (props) => {
    ...
    useEffect(() => {
        ...
        let timerId = setTimeout(async() => {
            try {
                await isOpen(props.route.params.restName, props.route.params.restaurantUid);
                controller = null;
            } catch (e) { 
              // Handle fetch error
            }
    ...
    async function isOpen (restName, restaurantUid) {
       ...
       const { currentUser } = firebase.auth();
        ...
        client.initIndex('foodsList').search(restaurantUid, {
                    ...
                    if (snapshot.val() !== null) {
                        if (props.route.params.compCameFrom === 'Notification') {
                            if (snapshot.val().restsList.indexOf(props.route.params.restaurantUid) !== -1) {
                                ...
                                // console.log('strFarAway1: ',strFarAway)
                                setData({ ...newData, farAway: strFarAway, foodMenu: object, restAxiosData: newResponse, cardRestNumFollowers: props.route.params.RestNumFollowers, cardIcon: 'heart', isLiked: true });
                            } else {
                                strFarAway = await distance(newResponse?.data?.result?.geometry?.location?.lat, newResponse?.data?.result?.geometry?.location?.lng);
                                // console.log('strFarAway: ',strFarAway)
                                setData({ ...newData, farAway: strFarAway, foodMenu: object, restAxiosData: newResponse, cardRestNumFollowers: props.route.params.RestNumFollowers, cardIcon: 'heart-o', isLiked: false });
                            }
                        } 
                    } else {                            
                        strFarAway = await distance(newResponse?.data?.result?.geometry?.location?.lat, newResponse?.data?.result?.geometry?.location?.lng);
                        // console.log('strFarAway: ',strFarAway)
                        setData({ ...newData, farAway: strFarAway, foodMenu: object, restAxiosData: newResponse, cardRestNumFollowers: props.route.params.RestNumFollowers });
                    }
                })
            } else {                            
                strFarAway = await distance(newResponse?.data?.result?.geometry?.location?.lat, newResponse?.data?.result?.geometry?.location?.lng);
            ...
            // setData({ ...newData, foodMenu: object, restAxiosData: newResponse, cardRestNumFollowers: props.route.params.RestNumFollowers });
        });
   }
...
// };   
    ...
    const followingFunc = () => {
      ...
      if (currentUser !== null) {
        ...
        const restListRef = firebase.database().ref(`/restsList/${props.route.params.objectID}`);
          ...
          >
            ...
            >
                <View style={{ padding: 20, marginTop: -30, marginBottom: 35, justifyContent: 'flex-start', marginLeft: -10, marginRight: -30 }}>
                    ...
                    <Text style={{ marginTop: 40, marginBottom: 5, fontFamily: 'MontserratBold', fontSize: 22, color: 'black' }}>{props.route.params.restName}</Text>
                    <Paragraph style={{ fontSize: 14, fontFamily: 'Montserrat', marginTop: 0, justifyContent: 'flex-start' }}>{props.route.params.restAddress}</Paragraph>
                        ...
                        props?.route?.params?.restDesc?.length !== 0 ?
                            <View style={{ flexDirection: 'row' }}>
                                <FontAwesome 
                                    ...
                                    size={20}
                                />
                                <Paragraph style={{ fontSize: 12, fontFamily: 'Montserrat', justifyContent: 'flex-start', color: 'rgba(0, 0, 0, 0.50)', marginBottom: 10, marginLeft: 10 }}>{props.route.params.restDesc}</Paragraph>
                            </View>
                        : null
                    }
                    <View style={{ marginTop: 10 }}>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginRight: 30 }}>
                                <View>
                                    {
                                        ...
                                        :
                                        <TouchableOpacity
                                            onPress={() => { Linking.openURL(`http://${props.route.params.restOrderWeb}`); }}
                                        > 
                                        <View style={{ flexDirection: 'row' }}>
                                        <FontAwesome 
                                            name="shopping-cart"
                                ...
                                <View>
                                    <TouchableOpacity
                                        onPress={() => { goToLocation(props.route.params.restAddress) }}
                                    > 
                                        <View style={{ flexDirection: 'row' }}>
                                        <FontAwesome 
                                            name="map-marker"
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginRight: 30 }}>
                                <View>
                                    <TouchableOpacity
                                        onPress={() => { Linking.openURL(`http://${props.route.params.restWebsite}`); }}
                                    > 
                                        <View style={{ flexDirection: 'row' }}>
                                        <FontAwesome 
                                            name="globe"
                                ...
                                <View>
                                    <TouchableOpacity
                                        onPress={() => { callNow(props.route.params.phoneNum) }}
                                    > 
                                        <View style={{ flexDirection: 'row' }}>
                                        <FontAwesome 
                                            name="phone"
