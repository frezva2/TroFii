...
import { View, Text, StyleSheet, Dimensions, TouchableOpacity, Image, FlatList, ActivityIndicator, SectionList, TouchableHighlight, Keyboard, Alert, Share } from 'react-native';
import { useTheme } from '@react-navigation/native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { StatusBar } from 'expo-status-bar';
import * as Linking from 'expo-linking';
import {  Avatar, Button, Card, Title, Paragraph, Searchbar, FAB, Caption, DefaultTheme } from 'react-native-paper';
...
const rateMeNow = async (item, index) => {
  ...
  if (userToken !== null) {
    setData({ ...data, isRatingNow: true, currentItem: item, currentIndex: index });
  } else {
...
const orderOnline = (restOrderWeb) => {
    ...
    const searchByFood = () => {
      ...
      Geocoder.from(data.myLocation).then((json) => {
          ...
          index.search(searchQuery,{
            ...
            restrictSearchableAttributes: ['foodInfo.food_name','foodInfo.tags','restName'],
            aroundRadius: Math.round(data.aroundRadius),
            getRankingInfo: true,  
            facetFilters: [`isRestActive:${true}`, `publish:${true}`, `foodInfo.foodType:${data.foodType}`]
          }).then(responses => {
              const str = JSON.stringify(responses.hits);
  ...
  const _renderItem = ({ item, index }) => {
    ...
    return (
      <ScrollView style={{ marginBottom: 1, marginTop: 1, width, height, marginLeft: -10 }}>
        <Card elevation={10} style={{ justifyContent: 'center', margin: 20, width: width * 0.82, borderRadius: 15, marginTop: 5, zIndex: -1 }}>
          { item?.userPostId !== undefined ?
            <TouchableOpacity 
              onPress={() => { gotoAccount(item?.realUserPostId) }}
              style={{ flex: 1, zIndex: 3, marginTop: 30, elevation: 10 }}
            >
              <Text style={{ 
                  color: 'white', 
          ...
          }
          {
            (item.foodInfo.image === oldImage || item.foodInfo.image === newImage)?
             <View /> : 
             <TouchableOpacity 
              onPress={() => { rateMeNow(item, index) }}
              style={{ flex: 1, zIndex: 2, borderRadius: 30, width: 50, height: 50, marginTop: item?.userPostId !== undefined ? -30 : 20, marginLeft: (height / width) > 1.5 ? width * 0.65 : width * 0.70, justifyContent: 'center', alignItems: 'center', elevation: 10, backgroundColor: 'white' }}
            >
               <View style={{ 
                    elevation: 10,
                    shadowColor: '#000',
          ...
          <View ref={(flatList) => { _flatListItem = flatList }} style={{ flex: 1, zIndex: -1, justifyContent: 'center', alignItems: 'center', marginTop: (item.foodInfo.image === oldImage || item.foodInfo.image === newImage) ? -5 : -75 }}>
            <Card.Cover style={{ flex: 1, zIndex: -1, width: width * 0.82, height: height * 0.4, borderTopLeftRadius: 15, borderTopRightRadius: 15 }} source={{ uri: (item.foodInfo.image === oldImage || item.foodInfo.image === newImage) ? newImage : item.foodInfo.image }} />
            {
              item.foodInfo.image === oldImage || item.foodInfo.image === newImage ?
                <TouchableOpacity 
          ...
          </View>
          {
            item?.comments ?
              <TouchableOpacity 
                style={{flex: 1, marginTop: -20, marginLeft: 15, marginBottom: -10 }} 
                onPress={() => {
                  const arr = Object.keys(item?.comments).map((key) => {
                    return (item?.comments[key]);
              ...
              >
                <View style={{ backgroundColor: 'white', borderRadius: 10, elevation: 3, width: 40, height: 40, justifyContent: 'center', alignItems: 'center' }} >
                  <Icon name="message-bulleted" size={30} color={'#595959'}/>
                </View>
              </TouchableOpacity>
              : <View />
          }       
          {
            (item.foodInfo.image === oldImage || item.foodInfo.image === newImage)?
              <View /> : 
              <TouchableOpacity
                  style={{ flex: 1, elevation: 1, marginLeft: width * 0.67, backgroundColor: 'white', height: 50, width: 50, borderRadius: 25, marginTop: item?.comments ? -35 : -25 }}
                  onPress={() => { shareNow(item.foodInfo.food_name, item.restName, item.restWebsite) }}
              > 
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginLeft: -10 }}>
            <View style={{ flexDirection: 'row', margin: 15, width: width * 0.57, justifyContent: 'flex-start' }}>
              ...
              <Card.Content style={{ marginTop: -5, marginLeft: -5 }}>
                ...
                <Text style={{ fontSize: 14, fontFamily: 'MontserratSemiBold', marginTop: 5 }}>{item.foodInfo.price !== 0 ? '$ ' : null}{item.foodInfo.price !== 0 ? item.foodInfo.price : null}</Text>
              </Card.Content>
            </View>
            <View style={{ marginTop: 15, alignItems: 'flex-start', marginLeft: 5 }}>
              <Caption style={{ fontFamily: 'MontserratBold', fontSize: 11, marginTop: 5, marginLeft: -25 }}>"{item.foodInfo.foodType}"</Caption>
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginLeft: -width * 0.12 }}>
                {
                  (item.totalView) === 0 ? <View /> : 
                  <View style={{ justifyContent: 'flex-start', alignItems: 'center', marginLeft: 1, flexDirection: 'row', marginTop: -5 }}>
                    <View>
                      <Button labelStyle={{ fontSize: 14, fontFamily: 'MontserratSemiBold', color: colors.text, marginLeft: 10, justifyContent: 'flex-start', alignItems: 'center' }} icon={require('../assets/icons/eye.png')} />
                    </View>
                    <View>
                      <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 11, color: colors.text, marginLeft: -28, marginTop: -2 }}>{(item.totalView) === 0 ? 'None' : item.totalView}</Text>
                    </View>
                  </View>
                }
                {
                  (item.foodInfo.Rate.qualityRate) === 0 ? <View /> : 
                  <View style={{ justifyContent: 'flex-start', alignItems: 'center', marginLeft: -25, flexDirection: 'row', marginTop: -5 }}>
                    <View>
                      <Button labelStyle={{ fontSize: 14, fontFamily: 'MontserratSemiBold', color: '#FFC607', marginLeft: 0, justifyContent: 'center', alignItems: 'center' }} icon={require('../assets/icons/star.png')} />
                    </View>
                    <View>
                      <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 11, color: '#FFC607', marginLeft: -22, marginTop: -2 }}>{(item.foodInfo.Rate.qualityRate) === 0 ? 'None' : item.foodInfo.Rate.qualityRate}</Text>
                    </View>
                  </View>
                }
              </View> 
          ...
          <View style={{ marginLeft: 15, marginTop: 5, marginRight: 10, marginBottom: 10 }}>
            {tagSeperator(item.foodInfo.tags)}
          </View>
          <Card.Actions style={{ width: width * 0.75, marginLeft: -15 }}>
          <TouchableOpacity onPress={() => { Linking.openURL(`http://${item.restWebsite}`); }}>
            <View style={{ alignItems: 'center', flexDirection: 'row', marginTop: -5 }}>
              ...
              <View>
                <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 11, color: colors.text, marginLeft: -25 }}>Website</Text>
              </View>
            </View>
          </TouchableOpacity>
          <TouchableOpacity  style={{ marginLeft: -5 }} onPress={() => { goToLocation(item.restAddress) }}>
            <View style={{ alignItems: 'center', flexDirection: 'row', marginTop: -5 }}>
              ...
              <View>
                <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 11, color: colors.text, marginLeft: -25 }}>Locate</Text>
              </View>
            </View>
          </TouchableOpacity>
          <TouchableOpacity style={{ marginLeft: -5 }} onPress={() => { callNow(item.phoneNum) }}>
            <View style={{ alignItems: 'center', flexDirection: 'row', marginTop: -5 }}>
              ...
              <View>
                <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 11, color: colors.text, marginLeft: -25 }}>Call</Text>
              </View>
            </View>
          </TouchableOpacity>
          <TouchableOpacity style={{ marginLeft: -5 }} onPress={() => { orderOnline(item.restOrderWeb) }}>
            <View style={{ alignItems: 'center', flexDirection: 'row', marginTop: -5 }}>
              ...
              <View>
                <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 11, color: colors.text, marginLeft: -25 }}>Order</Text>
              </View>
            </View>
          </TouchableOpacity>
          </Card.Actions>
          {/* {
            item.foodInfo.image === oldImage ?
             ...
             <Image
                  ...
                  source={require('../assets/icons/star-plus.png')}
                  fadeDuration={100}
                  />
            //  <FAB
            //    style={styles.fab}
    ...
    const tagsArr = tags.split(",");
      return ( 
        ...
        <Tags
          ...
          renderTag={({ tag, index }) => (
            <View key={`${tag}-${index}`} style={{ marginLeft: 2, marginTop: -10, marginRight: 3, marginBottom: 20, backgroundColor: 'rgba(238, 91, 100, 0.14)', borderRadius: 10, height: tag.length > width / 11 ? 50 : 30, justifyContent: 'center', alignItems: 'center' }}> 
    ...
    const renderHeader = () => {
      return (
        ...
        <View style={{ marginTop: 15, marginLeft: 25, marginBottom: 5 }}>
          <View style={{ flexDirection: 'row' }}>
            <FontAwesome 
                ...
                size={20}
            />
          <Caption style={{ fontFamily: 'Montserrat', fontSize: 14, marginLeft: 10 }}>{data.myLocation}</Caption>
          </View>
        </View>
        <View style={{ flexDirection: 'row', marginLeft: 25 }}>
          <View>
            <Text style={{ fontFamily: 'MontserratBold', fontSize: 24, color: colors.text }}>Search for </Text>
          </View>
          <View>
            <Text style={{ fontFamily: 'MontserratBold', fontSize: 24, color: '#EE5B64' }}>Food </Text>
          </View>
        </View>
        <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
          <View style={{ marginLeft: 25, marginRight: 5, marginTop: 15, marginBottom: 15, width: width * 0.72 }}>
            <Searchbar
        ...
        <View style={{ flexDirection: 'row', marginLeft: 25 }}>
          ...
          /> 
          <View>
            <Text style={{ fontFamily: 'Montserrat', fontSize: 12, color: colors.text, marginTop: -3, marginLeft: 10 }}>Sort by: </Text>
          </View>
          <View>
            <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 14, color: '#EE5B64', marginTop: -5, marginLeft: 1 }}>Best Food</Text>
          </View>
        </View>
        <View style={{ marginTop: 15, marginLeft: 25, marginBottom: 5 }}>
          <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 16, color: colors.text }}>Nearby Food</Text>
        </View>
      </View>
        )
    }
  ...
  const renderEmpty = () => {
      return (
      <View style={{ justifyContent: 'center', alignItems: 'center', marginLeft: 40, marginTop: 5 }}>
        <ScrollView style={{ marginBottom: 0, marginTop: 0, width, height }}>
          <Card elevation={10} style={{ justifyContent: 'center', margin: 20, width: width * 0.82, height: height * 0.8, borderRadius: 15, marginTop: 5, zIndex: -1, alignItems: 'center' }}>
          <View style={{ flex: 1, zIndex: -1, justifyContent: 'flex-start', alignItems: 'flex-start', marginTop: 0 }}>
            <ShimmerPlaceHolder style={{ flex: 1, zIndex: -1, width: width * 0.82, height: height * 0.5, borderTopLeftRadius: 15, borderTopRightRadius: 15, height: height * 0.25 }} />
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginLeft: -10 }}>
            <View style={{ flexDirection: 'row', margin: 15, width: width * 0.57, justifyContent: 'flex-start' }}>
              {/* <Avatar.Image size={50} source={{ uri: data.restsImageList[index] }} /> */}
              <Card.Content style={{ marginTop: -5, marginLeft: -5 }}>
                <ShimmerPlaceHolder style={{ width: width * 0.4, height: 15, borderRadius: 5 }} />
                <ShimmerPlaceHolder style={{ width: width * 0.2, height: 15, marginTop: 15, borderRadius: 5 }} />
                <ShimmerPlaceHolder style={{ width: width * 0.1, height: 15, borderRadius: 5, marginLeft: 0, marginTop: 15 }} />
              </Card.Content>
            </View>
            <View style={{ marginTop: 15, alignItems: 'flex-start', marginLeft: 5 }}>
              <ShimmerPlaceHolder style={{ width: width * 0.15, height: 15, borderRadius: 5 }} />
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginLeft: -width * 0.20 }}>
                <View style={{ justifyContent: 'flex-start', alignItems: 'center', marginLeft: 5, flexDirection: 'row', marginTop: 15 }}>
                  <View>
                    <ShimmerPlaceHolder style={{ width: 20, height: 20, borderRadius: 10 }} />
                  </View>
                  <View>
                    <ShimmerPlaceHolder style={{ width: width * 0.1, height: 15, borderRadius: 5, marginLeft: 5 }} />
                  </View>
                  <View>
                    <ShimmerPlaceHolder style={{ width: 20, height: 20, borderRadius: 10, marginLeft: 5 }} />
                  </View>
                  <View>
                    <ShimmerPlaceHolder style={{ width: width * 0.1, height: 15, borderRadius: 5, marginLeft: 5 }} />
                  </View>
          ...
          <View style={{ marginLeft: 15, marginTop: 25, marginRight: 10, marginBottom: 10 }}>
            <View style={{ flexDirection: 'row', marginLeft: 2, marginTop: 10, marginRight: 3, marginBottom: 20, borderRadius: 10, height: 3, justifyContent: 'center', alignItems: 'center' }}> 
              <ShimmerPlaceHolder style={{ width: width * 0.4, height: 30, borderRadius: 5 }} />
              <ShimmerPlaceHolder style={{ width: width * 0.2, height: 30, borderRadius: 5, marginLeft: 20 }} />
            </View>
            <View style={{ marginLeft: 10, marginTop: 20, marginRight: 3, marginBottom: 20, borderRadius: 10, height: 3, width: width * 0.5,justifyContent: 'center', alignItems: 'center' }}> 
              <ShimmerPlaceHolder style={{ width: width * 0.55, height: 30, borderRadius: 5 }} />
            </View>
            <View style={{ flexDirection: 'row', marginLeft: 2, marginTop: 20, marginRight: 3, marginBottom: 20, borderRadius: 10, height: 3, justifyContent: 'center', alignItems: 'center' }}> 
              <ShimmerPlaceHolder style={{ width: width * 0.15, height: 30, borderRadius: 5 }} />
              <ShimmerPlaceHolder style={{ width: width * 0.35, height: 30, borderRadius: 5, marginLeft: 20 }} />
            </View>
          </View>
          <Card.Actions style={{ width: width * 0.75, marginLeft: 10 }}>
            <View style={{ alignItems: 'center', flexDirection: 'row', marginTop: -5, marginRight: 10 }}>
              <View>
                <ShimmerPlaceHolder style={{ width: 20, height: 20, borderRadius: 10 }} />
              </View>
              <View>
        ...
        if (currentUser !== null) {
          if (list !== undefined) {
            if (data.finalResults[index].tempChecker.uids !== undefined) {
              if (hrs !== data.finalResults[index].tempChecker.hour && ((list.indexOf(`${currentUser.uid}`)) === -1) && (currentUser.uid !== data.finalResults[index].restaurantUid)) {
                firebase.database().ref(`/food/${data.finalResults[index].objectID}`)
                    .update({ 
              ...
              else if (data.finalResults[index].tempChecker.hour !== hrs && ((list.indexOf(`${currentUser.uid}`)) !== -1) && (currentUser.uid !== data.finalResults[index].restaurantUid)) {
                firebase.database().ref(`/food/${data.finalResults[index].objectID}`)
                    .update({ 
                      totalView: (data.finalResults[index].totalView + 1),
  ...
  const calcRating = (Rate) => {
    firebase.database().ref(`/food/${data.finalResults[data.currentIndex].objectID}`)
      .once('value', (snapshot) => {
        if(snapshot.val() !== null && snapshot.val() !== undefined ) {
          if (snapshot.val().foodInfo.Rate.totalRate === 0 || snapshot.val().foodInfo.Rate.qualityRate === 0) {
            setTimeout(() => {
              setData({ 
                ...data, 
                  currentRating: Rate,
            ...
            setTimeout(() => {
              setData({ 
                ...data, 
                  currentRating: Rate,
                  newRating: (((snapshot.val().foodInfo.Rate.qualityRate * snapshot.val().foodInfo.Rate.totalRate) + Rate) / (snapshot.val().foodInfo.Rate.totalRate + 1)).toFixed(2),
  ...
  const sendToAllNotification = () => {
    ...
    firebase.database().ref(`/users/uE1OWGdOQRfPna29DnQ9yGUTvIF3`).once("value").then((snapshot) => {
      if (snapshot.val() !== null) {
        sendPushNotification(snapshot.val().expoToken, 'uE1OWGdOQRfPna29DnQ9yGUTvIF3', 'New Rating ...', 
          `${data.finalResults[data.currentIndex].restName} got ${data.newRating}/5 rating for ${data.currentItem.foodInfo.food_name}. `, 
          'NewRating', data.currentItem.foodInfo.image, 
          currentUser.uid, data.currentItem.objectID);
      }
    });
  }
  const sendPushNotification = async (expoToken, expoTokenUid, title, body, type, image, uid, itemId) => {
     const year = new Date().getFullYear();
     const month = new Date().getMonth() + 1;
     const day = new Date().getDate();
     const hours = new Date().getHours();
     ...
     firebase.database().ref(`/users/${expoTokenUid}`).once('value', (snapshot) => {
       firebase.database().ref(`/users/${expoTokenUid}`)
         .update({  
          Notifications: 
            { 
  ...
  const submitNow = (foodType) => {
    ...
    dateID = ((year1 * 10000000000000) + (month1 * 100000000000) + 
      (day1 * 1000000000) + (hrs1 * 10000000) + (min1 * 100000) + (sec1 * 1000) + milsec1);
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Yes', onPress: () => {
            firebase.database().ref(`/food/${data.finalResults[data.currentIndex].objectID}/foodInfo/Rate`)
                  .update({ qualityRate: data.newRating, totalRate: data.newTotalRate });
            userRef.once('value', (userSnap) => {
              if (userSnap.val() !== null) {
                firebase.database().ref(`/food/${data.finalResults[data.currentIndex].objectID}/comments`)
                .push({ 
                ...
                firebase.database().ref(`/users/${currentUser.uid}`)
                 ...
                 firebase.database().ref(`/users/${data.currentItem.restaurantUid}`)
                     .once('value', (restSnap) => {
                         if (restSnap.val() !== null) {
                             firebase.database().ref(`/users/${currentUser.uid}/comments`)
                             .push({ 
                               comment: data.comment, 
                               dateComment: dateID, 
                               itemID: data.finalResults[data.currentIndex].objectID, 
                               ...
                               isMatched: data.isMatched,
                                ...
                                firebase.database().ref(`/food/${data.finalResults[data.currentIndex].objectID}/comments/${snapshot.key}`)
                                  .update({ key: snap.key });
                               });
                         }
                     });
              ...
              switch (foodType) {
                case ('Appetizer'): { 
                  ...
                  Alert.alert('Congratulations !', 'Your rate submitted successfully.', [{ text: 'OK', 
                    onPress: () => {
                      cancelRatings()
                      searchByFood();
                    }, 
  ...
  const cancelUpdatePic = () => {
    ...
    if (data.takenPicture !== '') {
      ...
      const newItem = data.finalResults;
      const imageUrl = update(data.finalResults[data.activeSlide], { foodInfo: { image: { $set: newImage } } });
      newItem[data.activeSlide] = imageUrl;
  ...
  const _renderComments = ({ item, index }) => {
    // console.log(item)
    return (
    <Card style={{ flex: 1, width: width * 0.95, justifyContent: 'center', alignItems: 'center', marginTop: 0, borderRadius: 15, elevation: 0, marginLeft: 10, marginBottom: 0, marginRight: 10 }}>
        <View style={{ alignItems: 'center', borderRadius: 10, justifyContent: 'center', marginTop: 10 }}>
          <View style={{ flex: 1, backgroundColor: 'white', alignItems: 'flex-start', justifyContent: 'flex-start', marginLeft: 5, flexDirection: 'row', borderRadius: 5 }}>
              <View style={{ height: 60, width: 60 }}>
                <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: 7.5, marginLeft: -2.5, position: 'absolute', height: 55, width: 55, borderRadius: 30, backgroundColor: '#e6e6e6', zIndex: 0 }} />
                <View style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'flex-start', marginTop: 10, zIndex: 1 }}>  
                    <Avatar.Image
                      size={50}
                      theme={styles.theme}
              ...
              <View style={{ flexDirection: 'row', width: width * 0.75 }}>
                <View style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'flex-start', marginLeft: 0, marginTop: 5 }}>
                    <View style={{ justifyContent: 'center', alignItems: 'center', marginBottom: 0 }}>
                        <Text style={{ fontFamily: 'MontserratBold', fontSize: 15, width: width * 0.65 }}>
                          {item.firstname} {item.lastname}
                        </Text>
                    </View>   
                    <View style={{ justifyContent: 'flex-start', alignItems: 'flex-start', marginBottom: 5, backgroundColor: 'rgba(0, 0, 0, 0.1)', borderRadius: 5, marginTop: 5 }}>
                        <Text 
                        ...
                        >
                          ...
                          {
                            item.isMatched ? 
                              <View style={{ flexDirection: 'row', marginTop: -5, marginLeft: 7, marginBottom: 5 }}>
                                <AirbnbRating
                                  count={5}
                                  reviews={["Terrible", "Not Too Bad", "Good", "Very Good", "Amazing"]}
                                ...
                                />
                                <Image
                                  style={{ marginLeft: 0, width: 15 , height: 15, marginTop: 3, marginRight: 10, marginLeft: 5 }}
            ...
            <View style={{ width: width * 0.70, justifyContent: 'space-evenly', alignItems: 'flex-start', marginBottom: 10, marginLeft: 30, flexDirection: 'row' }}>
            </View>  
        </View>
    </Card>
    );
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.container}>
          ...
          <View style={{ width, height: 56, justifyContent: 'center' }}>
            <View style={{ height: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <View style={{flex: 1, marginLeft: (height / width) > 1.5 ? width * 0.25 : width * 0.375, marginRight: 10 }} >
                  <Image
                    style={{ resizeMode: "contain", width: 47, height: 47, borderRadius: 5, marginTop: 3 }}
            ...
            <View style={{ width, marginTop: 15 }}>
              <View style={{ flexDirection: 'row', marginLeft: 25, marginBottom: 5 }}>
                <FontAwesome 
                    name="map-marker"
                    color={'rgba(0, 0, 0, 0.25)'}
                    size={20}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                <View style={{ marginLeft: 25, marginRight: 5, marginTop: 15, marginBottom: 15, width: width * 0.72 }}>
                  <Searchbar
                    placeholder="Search anything..."
                    onIconPress={() => { searchByFood(searchQuery); }}
                    onChangeText={onChangeSearch}
                    value={searchQuery}
                    // defaultValue={data.searchQuery}
                    onEndEditing={() => { searchByFood(searchQuery); }}
                    maxLength={100}
                    returnKeyType={'search'}
                    keyboardType={'default'}
                    selectionColor= '#EE5B64'
                    style={{ flex: 1, borderRadius: 15, width: width * 0.75 }}
                    inputStyle={{ fontFamily: 'Montserrat', fontSize: 14 }}
                  />
                </View>
                <TouchableOpacity
                  onPress={() => { setData({ ...data, isFilterChange: true }) }}
                  style={{ marginLeft: width * 0.02, marginTop: 0, marginBottom: 0 }}
                >
                  <Image
                    style={{ width: 70, height: 70, marginTop: 5 }}
                    source={require('../assets/icons/SQ.png')}
                ...
                <TouchableOpacity
                  onPress={() => { setData({ ...data, isSortByChange: true }) }}
                ...
                >
                  <View style={{ flexDirection: 'row', marginLeft: 25 }}>
                    ...
                    <View>
                      ...
                      <View>
                        <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 14, color: '#EE5B64', marginTop: -5, marginLeft: 1 }}>{checkSortBy()}</Text>
                      </View>
                  </View>
              </TouchableOpacity>
              <View style={{ marginTop: 15, marginLeft: 25, marginBottom: 5 }}>
            ...
            <View style={{ backgroundColor: 'white', flex: 1, marginBottom: 50 }}>
              {
                data.finalResults.length !== 0 ?
                  <Carousel 
                    ...
                    onSnapToItem={(index) => { totalViewFunc(index); }}
          ...
          {/* <Modal  
            ...
            propagateSwipe={true}
            swipeDirection={['down']}
            // onSwipeStart={(c) => {console.log('onSwipeStart', c)}}	
            // onSwipeMove={(c) => {console.log('onSwipeMove', c); }}		
            onSwipeCancel={(c) => {console.log('onSwipeCancel', c); setData({ ...data, newMargin: c.moveY }); }}
            // onSwipeComplete={(c) => {console.log('onSwipeComplete', c)}}
            // onSwipeComplete={() => { setData({ ...data, isSeeCommments: false }); }}	
            // onModalHide={() => { setData({ ...data, isSeeCommments: false }); }}
            // onModalShow={() => { setData({ ...data, isSeeCommments: true }); }}
            onBackdropPress={() => { setData({ ...data, isSeeCommments: false, newMargin: 0 }); }} 
            backdropColor='black'
            useNativeDriver={true}
            backdropOpacity={0.3}
            hideModalContentWhileAnimating
            onRequestClose={() => { setData({ ...data, isSeeCommments: false }); }} 
            style={{
        ...
        >
          <ModalContent>
              <View style={{  backgroundColor: 'white', marginLeft: -25 }}>
                <FlatList
                    ref={(flatList) => { _flatList1 = flatList }}
              ...
              <Modal1 
                ...
                onBackdropPress={() => { cancelUpdatePic() }} 
                backdropColor='black'
                useNativeDriver={true}
                backdropOpacity={0.3}
                hideModalContentWhileAnimating
              ...
              <View style={{ backgroundColor: 'white', alignItems: 'center', justifyContent: 'center' , borderRadius: 35 }}>
                ...
                <TouchableOpacity onPress={() => { postPhoto() }} disabled={data.takenPicture === '' ? true : false}>
                  <View style={{ marginTop: data.takenPicture === '' ? -20 : 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }}>
                ...
                <View style={{ marginTop: 5, justifyContent: 'center', alignItems: 'center', marginBottom: 15 }}>
                  <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center' }} onPress={() => { cancelUpdatePic() }} >
          ...
          <Modal1  
            ...
            propagateSwipe
            swipeDirectio={'up'}
            onModalHide={() => { setData({ ...data, isRatingNow: false }); }}
            onModalShow={() => { setData({ ...data, isRatingNow: true }); }}
            onBackdropPress={() => { cancelRatings() }} 
            backdropColor='black'
            useNativeDriver={true}
            backdropOpacity={0.3}
            hideModalContentWhileAnimating
            onRequestClose={() => { setData({ ...data, isRatingNow: false }); }} 
            style={{
              borderTopLeftRadius: 35,
              borderTopRightRadius: 35,
              borderBottomLeftRadius: 35,
          ...
          > 
            <ScrollView style={{ backgroundColor: 'white', borderRadius: 35, height: height * 0.70 }}>
              <TouchableOpacity
                onPress={() => { cancelRatings() }}
              ...
              <View style={{ marginTop: 0, justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{ fontFamily: 'MontserratBold', fontSize: 20, color: colors.text, justifyContent: 'center', alignItems: 'center'  }}>{data?.currentItem?.foodInfo?.food_name}</Text>
              </View>
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 15, marginBottom: 15, marginLeft: 0 }}>
                <Image
                    style={styles.CurrentImage3}
                    source={{ uri: data.currentItem?.foodInfo?.image, cache: 'force-cache' }}
                />
              </View>
              <View style={{ marginTop: -10, marginBottom: 0, marginLeft: width * 0.1 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 5 }}>
                <CheckBox
                  // center
                  onPress={() => { setData({ ...data, isMatched: 'yes' }) }}
                ...
                <CheckBox
                  // center
                  onPress={() => { setData({ ...data, isMatched: 'no' }) }}
                  containerStyle={{ width: width * 0.25 }}
                  title='NO'
                  checkedIcon='dot-circle-o'
                  uncheckedIcon='circle-o'
              ...
              />
              <View style={{ flexDirection: 'row', marginLeft: 30, marginBottom: 0, marginTop: 25 }}>
                <Icon name="comment-text" size={30} style={{ marginTop: 5 }}/>
                <View style={{ felx: 1 }}>
                    <Input
                      containerStyle={{ width: width * 0.70, marginTop: -10 }}
                      ...
                      value={data.comment}
                      onChangeText={(onChangeText) => { setData({ ...data, comment: onChangeText }) }}
                    />
                </View>
              </View>
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: -10 }}>
                ...
                <TouchableOpacity onPress={() => { cancelRatings() }} style={{ marginBottom: 30 }}>
                    <View style={{ marginTop: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }}>
          ...
          <Modal1  
            ...
            propagateSwipe
            swipeDirectio={'up'}
            onModalHide={() => { setData({ ...data, isSortByChange: false }); }}
            onModalShow={() => { setData({ ...data, isSortByChange: true }); }}
            onBackdropPress={() => { setData({ ...data, isSortByChange: false, sortBy: data.oldSortBy }); }} 
            onSwipeComplete={() => { setData({ ...data, isSortByChange: false }); }} 
            backdropColor='black'
            useNativeDriver={true}
            backdropOpacity={0.3}
            hideModalContentWhileAnimating
            onRequestClose={() => { setData({ ...data, isSortByChange: false }); }} 
            style={{
                borderTopLeftRadius: 35,
                borderTopRightRadius: 35,
                // borderBottomLeftRadius: 35,
          ...
          >
            <View style={{ marginBottom: -height/2, borderTopLeftRadius: 35, borderTopRightRadius: 35, backgroundColor: 'white', height: height * 0.50, width, marginLeft: 0, width }}>
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: -15 }}>
                <Text style={{ fontFamily: 'MontserratBold', fontSize: 24, color: '#EE5B64' }}>_____</Text>
              </View>
              <View style={{ marginTop: 10, marginLeft: 25 }}>
                <Text style={{ fontFamily: 'MontserratBold', fontSize: 24, color: colors.text }}>Sort By</Text>
              </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', marginTop: 15 }}>
                  <CheckBox
                    // center
                    onPress={() => { setData({ ...data, sortBy: 'sortByTotalView' }) }}
                  ...
                  <CheckBox
                    // center
                    title='Best Food'
                    onPress={() => { setData({ ...data, sortBy: 'sortByTotalRate' }) }}
                    containerStyle={{ width: width * 0.45 }}
                    checkedIcon='dot-circle-o'
                    uncheckedIcon='circle-o'
                    checkedColor='#EE5B64'
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', marginTop: 15 }}>
                  <CheckBox
                    // center
                    onPress={() => { setData({ ...data, sortBy: 'worests' }) }}
                    containerStyle={{ width: width * 0.45 }}
                  ...
                  <CheckBox
                    // center
                    onPress={() => { setData({ ...data, sortBy: 'sortByPrice' }) }}
                    containerStyle={{ width: width * 0.45 }}
              ...
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 25 }}>
                <TouchableHighlight underlayColor="white" onPress={() => { searchByFood() }}>
                  {/* <View style={{ marginTop: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }}> */}
          ...
          <Modal1  
            ...
            propagateSwipe
            swipeDirectio={'up'}
            onModalHide={() => { setData({ ...data, isFilterChange: false }); }}
            onModalShow={() => { setData({ ...data, isFilterChange: true }); }}
            onBackdropPress={() => { setData({ ...data, isFilterChange: false }); }} 
            onSwipeComplete={() => { setData({ ...data, isFilterChange: false }); }} 
            backdropColor='black'
            useNativeDriver={true}
            backdropOpacity={0.3}
            hideModalContentWhileAnimating
            onRequestClose={() => { setData({ ...data, isFilterChange: false }); }} 
            style={{
                borderTopLeftRadius: 35,
                borderTopRightRadius: 35,
                // borderBottomLeftRadius: 35,
          ...
          >
            <View style={{ marginBottom: -height/2, borderTopLeftRadius: 35, borderTopRightRadius: 35, backgroundColor: 'white', height: height * 0.65, width, marginLeft: 0, width }}>
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: -15 }}>
                <Text style={{ fontFamily: 'MontserratBold', fontSize: 24, color: '#EE5B64' }}>_____</Text>
              </View>
              <View style={{ marginTop: 10, marginLeft: 25 }}>
                <Text style={{ fontFamily: 'MontserratBold', fontSize: 24, color: colors.text }}>Filters</Text>
              </View>
              <View style={{ marginTop: 10, marginLeft: 25 }}>
                <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 16, color: colors.text }}>Type</Text>
              </View>
              <View style={{ flexDirection: 'row' }}>
                <Button mode="contained" onPress={() => setData({ ...data, foodType: '' })} labelStyle={{ fontFamily: 'MontserratBold', fontSize: 12, color: 'white' }} style={{ marginLeft: 25, marginTop: 15, borderRadius: 15, backgroundColor: data.foodType === '' ? '#EE5B64' : '#707070' }}>
                  All
                </Button>
                <Button mode="contained" onPress={() => setData({ ...data, foodType: 'Drink' })} labelStyle={{ fontFamily: 'MontserratBold', fontSize: 12 }} style={{ marginLeft: 10, marginTop: 15, borderRadius: 15, backgroundColor: data.foodType === 'Drink' ? '#EE5B64' : '#707070' }}>
                  Drink
                </Button>
                <Button mode="contained" onPress={() => setData({ ...data, foodType: 'Appetizer' })} labelStyle={{ fontFamily: 'MontserratBold', fontSize: 12 }} style={{ marginLeft: 10, marginTop: 15, borderRadius: 15, backgroundColor: data.foodType === 'Appetizer' ? '#EE5B64' : '#707070' }}>
                  Appetizer
                </Button>
              </View>
              <View style={{ flexDirection: 'row' }}>
                <Button mode="contained" onPress={() => setData({ ...data, foodType: 'Dessert' })} labelStyle={{ fontFamily: 'MontserratBold', fontSize: 12, color: 'white' }} style={{ marginLeft: 25, marginTop: 15, borderRadius: 15, backgroundColor: data.foodType === 'Dessert' ? '#EE5B64' : '#707070' }}>
                  Dessert
                </Button>
                <Button mode="contained" onPress={() => setData({ ...data, foodType: 'Entrée' })} labelStyle={{ fontFamily: 'MontserratBold', fontSize: 12 }} style={{ marginLeft: 15, marginTop: 15, borderRadius: 15, backgroundColor: data.foodType === 'Entrée' ? '#EE5B64' : '#707070' }}>
                  Entrée
                </Button>
              </View>
              <View style={{ marginTop: 20, marginLeft: 25 }}>
                <Text style={{ fontFamily: 'MontserratSemiBold', fontSize: 16, color: colors.text }}>Location</Text>
              </View>
              <TouchableOpacity
                  onPress={() => { setData({ ...data, isLocationChange: true }) }}
                  style={{ marginVertical: 10, marginLeft: 0, marginTop: 5, marginBottom: 5 }}
              >
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 30 }}>
                ...
                <TouchableHighlight underlayColor="white" onPress={() => { resetFilter() }}>
                  {/* <View style={{ marginTop: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }} > */}
                      <LinearGradient colors={['white', 'white', 'white']} 
                        style={styles.linearGradient3}
                      >
                        <Text style={{ fontFamily: 'MontserratBold', fontSize: 14, color: '#EE5B64', marginLeft: 10, marginRight: 30, marginLeft: 30 }}>Reset</Text>
                      </LinearGradient>
                  {/* </View> */}
                </TouchableHighlight>   
              </View>
          ...
          <Modal1  
              ...
              animationOutTiming={550} 
              propagateSwipe
              onModalHide={() => { setData({ ...data, isNoResults: false }); }}
              onModalShow={() => { setData({ ...data, isNoResults: true }) }}
              onBackdropPress={() => { setData({ ...data, isNoResults: false }); }} 
              backdropColor='black'
              useNativeDriver={true}
              backdropOpacity={0.3}
              hideModalContentWhileAnimating
              onRequestClose={() => { setData({ ...data, isNoResults: false }); }} 
              style={{
                  borderTopLeftRadius: 35,
                  borderTopRightRadius: 35,
                  borderBottomLeftRadius: 35,
          ...
          >
          <View style={{ backgroundColor: 'white' , borderRadius: 35, height: height * 0.75 }}>
            <TouchableOpacity
                  onPress={() => { setData({ ...data, isNoResults: false }); }}
                  style={{ marginVertical: 10, marginLeft: 25, marginTop: 25 }}
              >
                  <Feather 
                      name="x"
              ...
              <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                ...
                <View>
                    <Title style={{ color: 'black', fontFamily: 'MontserratBold', fontSize: 18, marginTop: 10 }}>No Restaurants</Title>
                </View>
                <View style={{ width: width * 0.8 }}>
                    <Title style={styles.buttonText4}>Unfortunately, we cannot find any restaurants or food around your location matching your request.</Title>
                </View>
              </View>
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 30 }}>
                <TouchableOpacity onPress={() => { setData({ ...data, isNoResults: false }) }} >
                    <View style={{ marginTop: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }}>
                        <LinearGradient colors={['#fb8389', '#f70814', '#C90611']} style={styles.linearGradient}>
