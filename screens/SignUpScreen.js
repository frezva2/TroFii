...
import {  Avatar, Card, Title, Paragraph, DefaultTheme, useTheme } from 'react-native-paper';
...
const SignUpScreen = ({ navigation }) => {
    ...
    const resetMyPass = () => {
		firebase.auth().sendPasswordResetEmail(newData.email, null)
			.then(() => {
                setData({
                    ...newData,
                    isValidEmail: false,
	...
	const loginHandle = () => {
		...
		const { firstname, lastname, email, password, image, points, tempHours, aroundRadius, code, redeemedPoints, Notifications, iseGiftRequested, followingList,
			username, tokenPass, restName, yourLocation, followerNum, userTotalRate, restMaxPercentage, phoneNum, restDesc, egiftChoice, followersList, restFollowingNum,
			restAddress, isRestActive, restOrderWeb, restWebsite, isRecommended, restHours, egiftEarned, eGiftChoiceDate, boolToken } = newData;
			if (newData.yourLocation === null || newData.yourLocation === '' || newData.yourLocation === ' ') {
	    		Alert.alert(`Location's Selection Error!`, 'Please select your location.', [{ text: 'OK', 
					onPress: () => {}, 
				...
				firebase.auth().createUserWithEmailAndPassword(email, password)
							...
							currentUser.sendEmailVerification().then(() => {
								if (currentUser.uid !== null) {
									...
									firebase.database().ref(`/users/${currentUser.uid}`)
										.update({ firstname, lastname, email, isUndecided: false, image, restFollowingNum, followersList: { 0: '_' }, followingList: { 0: '_' }, eGiftChoiceDate, iseGiftRequested, egiftChoice, Notifications, code, egiftEarned, restDesc, phoneNum, restHours, isRecommended, username, redeemedPoints, userTotalRate, tokenPass, restMaxPercentage, tempHours, restName, yourLocation, followerNum, followersList: { 0: '_' }, restsList: { 0: '_' }, points, restAddress, restOrderWeb, restWebsite, isRestActive, aroundRadius, boolToken, createdAt: createdDate });	
                                    // signIn();
					...
					.catch((error) => {
						...
						if (errorCode === 'auth/user-disabled') {
							setData({ ...newData, errorMessage: 'Your email is currently disabled, please contact us.' });
						} else if (errorCode === 'auth/invalid-email') {
							setData({ ...newData, errorMessage: 'Your email address is not valid, please try again.' });
						} else if (errorCode === 'auth/wrong-password') {
							setData({ ...newData, errorMessage: 'Your password is not correct, please try again.' });
						} else if (errorCode === 'auth/user-not-found') {
							setData({ ...newData, errorMessage: 'We cannot find your email, please create an account with us.' });
						} else if (errorCode === 'auth/invalid-email-verified') {
							setData({ ...newData, errorMessage: 'Please, verify your email, we already sent you a verification email.' });
						} else if (errorCode === 'auth/email-already-in-use') {
							setData({ ...newData, errorMessage: 'This email address is already exist, please try a different email address.' });
						}else if (errorCode === 'auth/weak-password') {
							setData({ ...newData, errorMessage: 'Your password is week, please try a stronger password.' });
						}
					});
			}
	}
      ...
      <View style={styles.container}>
        ...
        //   backgroundColor='#ed5962' 
          barStyle="light-content"/>
            <ImageBackground source={require('../assets/icons/signup_top.png')} resizeMode="cover" style={styles.image} />
            <Animatable.View 
            ...
            >
                ...
                >
                    ...
                    />
                    <Text style={{ marginTop: 40, marginBottom: 5, fontFamily: 'MontserratSemiBold', fontSize: 24, color: 'black', marginLeft: -20 }}>Sign Up</Text>
                    <Text style={{ marginTop: 20, marginBottom: 5, fontFamily: 'MontserratSemiBold', fontSize: 14, color: '#C90611' }}>FIRST NAME</Text>
                    ...
                    <Text style={{ marginTop: 20, marginBottom: 5, fontFamily: 'MontserratSemiBold', fontSize: 14, color: '#C90611' }}>LAST NAME</Text>
                    <View style={styles.action}>
                        <FontAwesome 
                            name="user-o"
                            color={colors.text}
                    ...
                    <Text style={{ marginTop: 20, marginBottom: 5, fontFamily: 'MontserratSemiBold', fontSize: 14, color: '#C90611' }}>LOCATION</Text>
                    <TouchableOpacity onPress={() => { setData({ ...newData, isChangeLocation: true }); }} >
                        <View style={styles.action}>
                            <FontAwesome 
                                name="map-marker"
                                color={colors.text}
                                size={20}
                            />
                            <Text style={{ marginLeft: 15, marginBottom: 5, fontFamily: 'MontserratSemiBold', fontSize: 14, color: 'black' }}>{newData.yourLocation}</Text>
                        </View>
                    </TouchableOpacity>
                        { 
                            newData.isEnterLocation === false ? null : 
                                <Animatable.View animation="fadeInLeft" duration={500} style={{ flex: 1, marginLeft: -10, marginTop: 10 }}>
                                    ...
                                    onPress={(data, details = null) => { // 'details' is provided when fetchDetails = true
                                        setData({ ...newData, yourLocation: data.description, isChangeLocation: false, isEnterLocation: false });
                                    }}
                                    // getDefaultValue={() => {
                        ...
                        <View style={styles.button} >
                            <TouchableOpacity onPress={() => { loginHandle( newData.email, newData.password ) }}>
                        ...
                        {/* { 
                            newData.isValidEmail === false ? null : 
                                <Animatable.View animation="fadeInLeft" duration={500}>
                        ...
                        {/* { 
                            newData.isReqResetPass === false ? null : 
                                <Animatable.View animation="fadeInLeft" duration={500}>
                    ...
                    <View style={styles.textPrivate2}>
                        ...
                        <TouchableOpacity 
                            onPress={() => { 
                                // props.navigation.navigate({name:'SignInScreen', params:{ routeCameFrom: props?.route?.params?.routeCameFrom, merge: true }}) 
                                navigation.navigate('SignInScreen', 
                            // {
                            //     screen: 'SignInScreen',
                                // params: {
                            ...
                            <Text style={[styles.color_textPrivateBold2, {fontWeight: 'bold'}]}>{" "}Sign In</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{ flex: 1 }}>
                    <Modal  
                        ...
                        propagateSwipe
                        onModalHide={() => { setData({ ...newData, isChangeLocation: false }); }}
                        onModalShow={() => { setData({ ...newData, isChangeLocation: true }) }}
                        onBackdropPress={() => { setData({ ...newData, isChangeLocation: false }); }} 
                        backdropColor='black'
                        useNativeDriver={true}
                        backdropOpacity={0.3}
                        hideModalContentWhileAnimating
                        onRequestClose={() => { setData({ ...newData, isChangeLocation: false }); }} 
                        style={{
                    ...
                    <TouchableOpacity
                            onPress={() => { setData({ ...newData, isChangeLocation: false }); }}
                            style={{ marginVertical: 10, marginLeft: -width/2 -60, marginTop: 25 }}
                        >
                            <Feather 
                                name="x"
                                color="grey"
                        ...
                        <View>
                            <Title style={{ color: 'black', fontFamily: 'MontserratBold', fontSize: 18, marginTop: 10 }}>Enable Your Location</Title>
                        </View>
                        <View style={{ marginTop: 0, marginLeft: 0, width: width * 0.90, borderRadius: 35 }}>
                            <View style={{ marginTop: 5, alignItems: 'flex-start', justifyContent: 'flex-start',  marginBottom: 5, backgroundColor: 'white' }}>               
                                <View style={{ width: width * 0.75, marginTop: 0, marginLeft: 15 }}>
                                    <Paragraph style={styles.searchDescStyle}>This app requires your location. You can manually enter it or enable your location settings to automatically detect your location.</Paragraph>
                                </View>
                            </View>
                        </View>
                        </View>
                        ...
                        <TouchableOpacity onPress={() => { setData({ ...newData, isEnterLocation: true, isChangeLocation: false }) }} style={{ marginBottom: 30 }}>
                            <View style={{ marginTop: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 5 }}>
